...
```
【OpenAPI仕様書を添付】openapi.yaml

【プロンプト例】
以下のテストシナリオを実装してください:
1. APIにログインしてトークンを取得
2. ランダムなハッシュ値を含む商品名でサンプル注文を作成
3. 注文取得APIで正しく取得できることを確認
4. 先に発送処理を実行し、400エラーになることを確認
5. 支払い処理を実行し、ステータスがpaidになることを確認
6. 発送処理を実行し、ステータスがshippedになることを確認
```
...
| フェーズ | 実行方法 | 操作内容 |
|---------|---------|---------|
| **セットアップ** | API | POST /api/reset - データ初期化 |
| | API | POST /login - トークン取得 |
| | API | POST /api/orders - 注文作成 |
| **UI操作** | UI | 注文管理画面を開く |
| | UI | 支払いボタンをクリック |
| **検証** | API | GET /api/orders/1 - status確認 |
| | API | Assert: status === "paid" |
